...
export interface PropertiesTableProps {
  ...
  sortField: keyof AppProperty | string;
  sortOrder: SortOrder;
  toggleSort: (key: keyof AppProperty | string) => void;
}
...
export default function PropertiesTable({
   ...
   */
  const normalized = React.useMemo(() => properties.map((p) => {
  ...
  }), [properties, propertyTypesMap]);

  // Definir columnas para ReusableTable
  const columns: Column<AppProperty & { ownerName: string; typesOfServiceStr: string }>[] = [
    ...
    },
    {
      ...
      headerClassName: "px-2 py-2 text-xs text-center",
      cellClassName: "px-2 py-2 text-xs text-center",
      render: (property) => <div className="truncate text-xs"><TruncatedText text={property.name || ""} maxLength={20} /></div>,
    },
    {
      key: "address",
      ...
      render: (property) => (
        <button
          ...
          className="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer bg-transparent border-none p-0 font-normal truncate text-xs block max-w-[140px] mx-auto whitespace-nowrap overflow-hidden"
          title="Ver en mapa"
          aria-label={getText("properties.table.viewMap", "Ver en mapa")}
        >
          <TruncatedText text={property.address || ""} maxLength={30} />
        </button>
      ),
    },
    {
      ...
      headerClassName: "px-2 py-2 text-xs text-center",
      cellClassName: "px-2 py-2 text-xs text-center",
      render: (property) => <div className="truncate text-xs"><TruncatedText text={property.typesOfServiceStr || "-"} maxLength={20} /></div>,
    },
    {
      key: "shiftsCount",
  ...
  const renderActions = (property: AppProperty & { ownerName: string; typesOfServiceStr: string }) => {
    ...
    // Modo desplegado: todos los botones en una sola fila (sin wrap)
    return (
      <div className="flex gap-2 justify-center items-center whitespace-nowrap">
        ...
        <Button
          ...
          aria-label={getText("actions.view", "Ver detalles")}
  ...
  return (
    <>
      <ReusableTable
        ...
        sortOrder={sortOrder}
        toggleSort={(field) => {
          // Permitir orden para claves reales o mapear si necesario
          toggleSort(field);
        }}
