...
export default function PropertyServiceEdit({ service, open, onClose, onUpdated, compact = false }: PropertyServiceEditProps) {
  const { TEXT } = useI18n();
  ...
  const [totalHours, setTotalHours] = React.useState<string>(() => service.totalHours ?? "");
  const [contractStartDate, setContractStartDate] = React.useState<string>(() => service.contractStartDate ?? "");
  const [plannedStart, setPlannedStart] = React.useState<string>(() => service.startTime ?? "");
  const [plannedEnd, setPlannedEnd] = React.useState<string>(() => service.endTime ?? "");
  ...
  const [lastModifiedField, setLastModifiedField] = React.useState<'rate' | 'hours' | 'budget' | null>(null);
  ...
  const [scheduleType, setScheduleType] = React.useState<'specific' | 'period'>('specific');
  const [scheduleView, setScheduleView] = React.useState<'classic' | 'calendar'>('classic');
  ...
  const [calendarStartTime, setCalendarStartTime] = React.useState<string>("09:00");
  const [calendarEndTime, setCalendarEndTime] = React.useState<string>("17:00");
  
  // Estado para manejar horas individuales de cada fecha
  const [dateTimeMap, setDateTimeMap] = React.useState<{[date: string]: {start: string, end: string}}>({});
  
  // Estado para el filtro de fechas
  const [dateFilter, setDateFilter] = React.useState<'day' | 'week' | 'month'>('day');

  // Estado para tracking de período actual en modo período
  const [currentPeriodStart, setCurrentPeriodStart] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    const t = setTimeout(() => setPropertySearchTerm(propertySearchInput.trim()), 300);
  ...
  }, [totalHours, rate, lastModifiedField, isCalculating, monthlyBudget, calculateMonthlyBudget, cleanCurrency]);
  ...
  const getDateTimes = (date: string) => {
    return dateTimeMap[date] || { start: calendarStartTime, end: calendarEndTime };
  };
  ...
  const getWeekdayDatesInRange = (referenceDate: Date, startDate: Date, endDate: Date): string[] => {
  ...
  const getMonthdayDatesInRange = (referenceDate: Date, startDate: Date, endDate: Date): string[] => {
  ...
  const generatePeriodDates = (startDate: string, endDate: string): string[] => {
  ...
  const replicateWeeklyPattern = (selectedDates: string[]): string[] => {
    ...
    if (plannedStart && plannedEnd) {
      ...
      startDate = new Date(firstDate.getFullYear(), firstDate.getMonth() - 1, 1).toISOString().split('T')[0];
      endDate = new Date(firstDate.getFullYear(), firstDate.getMonth() + 5, 0).toISOString().split('T')[0];
    }
  ...
  const replicateMonthlyPattern = (selectedDates: string[]): string[] => {
    ...
    if (plannedStart && plannedEnd) {
      ...
      const firstDate = new Date(selectedDates[0]);
      startDate = new Date(firstDate.getFullYear() - 1, firstDate.getMonth(), 1).toISOString().split('T')[0];
      endDate = new Date(firstDate.getFullYear() + 1, firstDate.getMonth(), 0).toISOString().split('T')[0];
    }
  ...
  const generateScheduleSummary = (): string => {
    ...
    const dates = validDates.map(d => new Date(d)).sort((a, b) => a.getTime() - b.getTime());
    ...
    if (plannedStart && plannedEnd) {
      summary += `Servicio planificado del ${new Date(plannedStart).toLocaleDateString('es-ES')} al ${new Date(plannedEnd).toLocaleDateString('es-ES')}. `;
    }
    ...
    if (scheduleType === 'specific') {
      ...
      summary += `Programado por período del ${periodStart ? new Date(periodStart).toLocaleDateString('es-ES') : ''} al ${periodEnd ? new Date(periodEnd).toLocaleDateString('es-ES') : ''}. `;
    }
    ...
    if (totalDates === 1) {
      ...
      summary += `${totalDates} fechas desde ${firstDate.toLocaleDateString('es-ES')} hasta ${lastDate.toLocaleDateString('es-ES')}. `;
    }
    ...
    if (workingDays.length > 0 && totalDates > 2) {
      ...
      const getConsecutiveRanges = (days: number[]): string => {
        ...
        if (ranges.length === 1) {
          ...
          return ranges.slice(0, -1).join(', ') + ' y ' + ranges[ranges.length - 1];
        }
      };
      ...
      if (isWeekdays && workingDays.length === 5 && workingDays.join(',') === '1,2,3,4,5') {
        ...
        dayPattern = `principalmente ${mostCommonDays.join(', ').replace(/, ([^,]*)$/, ' y $1')}`;
      }
  ...
  const isDateDisabled = (date: Date): boolean => {
    ...
    const [startYear, startMonth, startDay] = plannedStart.split('-').map(Number);
  ...
  const handleModalWidthChange = (newWidth: number) => {
    ...
    localStorage.setItem('propertyServiceEdit-modalWidth', clampedWidth.toString());
  };
  ...
  const handleMonthlyBudgetChange = (value: string) => {
    ...
    if (isValidNumber(cleaned) || cleaned === '') {
      ...
      if (parseFloat(cleaned) > 0 && parseFloat(cleanCurrency(rate)) > 0) {
        setRate('0');
        toast.info("Tarifa reseteada a $0 por modificación manual del presupuesto");
      }
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    payload.monthly_budget = monthlyBudget === "" ? undefined : cleanCurrency(monthlyBudget);
  ...
  const propertyLabel = (p: AppProperty) => {
    ...
    return `${primaryName}${secondaryName}${p.address ? ` — ${p.address}` : ''}`;
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className={dialogClass} style={dialogStyle} ref={dialogRef}>
        ...
        <DialogHeader>
          <DialogTitle className={titleClass}>{TEXT?.services?.edit?.title ?? "Edit Service"} — Property View</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div data-property-dropdown>
                ...
                <div className="relative">
                  <Input
                    ...
                    readOnly
                    placeholder={TEXT?.services?.placeholders?.property ?? "Click to select property"}
                    ...
                    <div className="absolute z-50 mt-1 w-full rounded-md border bg-popover shadow-lg max-h-60 overflow-hidden text-sm">
                      ...
                      <div className="p-2 border-b">
                        <Input
                          ...
                          onChange={(e) => setPropertySearchInput(e.currentTarget.value)}
                      ...
                      <div className="max-h-40 overflow-auto">
                        {propsQuery.isFetching && (
                          <div className="p-2 text-sm text-muted-foreground">{TEXT?.common?.loading ?? "Loading..."}</div>
                        )}
                        
                        {Array.isArray(propsQuery.data) && propsQuery.data.length > 0 ? (
                          propsQuery.data.map((p) => (
                            ...
                            >
                              <div className="font-medium">{p.alias || p.name || `Property #${p.id}`}</div>
                              {p.alias && p.name && (
                          ...
                          !propsQuery.isFetching && (
                            <div className="p-2 text-sm text-muted-foreground">
                              {propertySearchInput ? "No se encontraron propiedades" : "No hay propiedades disponibles"}
                            </div>
                          )
                        )}
                      </div>
                  ...
                  {propertyId !== null && (
                    <button type="button" aria-label="Clear property" onClick={clearPropertySelection} className="absolute right-8 top-2 text-sm opacity-70">✕</button>
                  )}
              ...
              <div className="grid grid-cols-4 gap-2">
                <div>
                  <label className="text-sm">{TEXT?.services?.fields?.totalHours ?? "Cant. Hrs"}</label>
                  <Input 
                    value={totalHours} 
                    onChange={(e) => handleTotalHoursChange(e.currentTarget.value)} 
                    placeholder="0" 
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input 
                    ...
                    onChange={(e) => handleMonthlyBudgetChange(e.currentTarget.value)}
                ...
                <div>
                  <label className="text-sm">{TEXT?.services?.fields?.contractStartDate ?? "Contract Date"}</label>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-sm">{TEXT?.services?.fields?.startTime ?? "Start"}</label>
                  <Input type="date" value={plannedStart} onChange={(e) => setPlannedStart(e.currentTarget.value)} />
                </div>
                <div>
                  <label className="text-sm">{TEXT?.services?.fields?.endTime ?? "End"}</label>
                  <Input type="date" value={plannedEnd} onChange={(e) => setPlannedEnd(e.currentTarget.value)} />
                </div>
              </div>
              ...
              {schedule.length > 0 && (
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-blue-900 mb-1">Resumen del Servicio</h4>
            ...
            <div className="lg:col-span-3 space-y-4">
              <div className="p-4">
                <div className="mb-4 grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-sm font-medium mb-2 block">Tipo de schedule</label>
                    <Select value={scheduleType} onValueChange={(value: 'specific' | 'period') => setScheduleType(value)}>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Seleccionar tipo" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="specific">Fechas específicas</SelectItem>
                  ...
                  <div>
                    ...
                    <Select value={scheduleView} onValueChange={(value: 'classic' | 'calendar') => setScheduleView(value)}>
                      <SelectTrigger className="w-full">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Checkbox 
                      ...
                      onCheckedChange={(v) => {
                        ...
                        // Si se desmarca recurrent, resetear el filtro a 'day'
                        if (!v) {
                          setDateFilter('day');
                        }
                      }} 
                  ...
                  {scheduleView === 'classic' ? (
                    ...
                    scheduleType === 'specific' ? (
                      ...
                      <>
                        <div>
                          <label className="text-sm text-muted-foreground mb-2 block">Schedule dates</label>
                          <div className="flex gap-2 items-center">
                            <Input type="date" value={scheduleInput} onChange={(e) => setScheduleInput(e.currentTarget.value)} />
                            <Button variant="outline" size="sm" onClick={addScheduleDate} disabled={!scheduleInput}>
                        ...
                        {schedule.length > 0 && (
                          <div className="space-y-3">
                            ...
                            <div className="bg-muted/20 rounded p-3 border">
                              <div className="flex justify-between items-center">
                                <span className="text-sm font-medium text-muted-foreground">
                                  Total de fechas programadas: {schedule.length}
                                </span>
                            ...
                            <div>
                              <label className="text-sm text-muted-foreground mb-2 block">Added dates</label>
                              <ul className="space-y-1 text-xs max-h-32 overflow-y-auto">
                                {schedule.map((d) => (
                                  <li key={d} className="flex justify-between items-center gap-2 bg-background rounded px-2 py-1">
                                    <span>{d}</span>
                                    <button type="button" className="text-sm opacity-80 hover:opacity-100" onClick={() => removeScheduleDate(d)}>✕</button>
                                  </li>
                                ))}
                              </ul>
                            </div>
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <Input type="date" value={periodStart} onChange={(e) => setPeriodStart(e.currentTarget.value)} />
                          </div>
                          <div>
                            <label className="text-sm text-muted-foreground mb-2 block">Fecha fin</label>
                            <Input type="date" value={periodEnd} onChange={(e) => setPeriodEnd(e.currentTarget.value)} />
                          </div>
                        </div>
                        ...
                        {schedule.length > 0 && (
                          <div>
                            <label className="text-sm text-muted-foreground mb-2 block">Fechas generadas ({schedule.length})</label>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <label className="text-sm text-muted-foreground">
                          Seleccionar fechas en calendario ({schedule.length} seleccionadas)
                        </label>
                        {schedule.length > 0 && (
                          <Button 
                            variant="outline" 
                          ...
                          <div className="space-y-3">
                            <Calendar
                              ...
                              onSelect={(clickedDate) => {
                                if (clickedDate) {
                                  const clickedDateStr = convertDatesToStringArray([clickedDate])[0];
                                  ...
                                  if (scheduleType === 'period') {
                                    if (schedule.includes(clickedDateStr)) {
                                      ...
                                      const newDates = schedule.filter(d => d !== clickedDateStr);
                                      setSchedule(newDates);
                                      ...
                                      if (!currentPeriodStart) {
                                        ...
                                        setSchedule([...schedule, clickedDateStr].sort());
                                      } else {
                                        // Ya hay inicio definido: esta fecha es el final del período
                                        const rangeLength = getDatesInRange(currentPeriodStart, clickedDateStr).length;
                                        
                                        if (rangeLength <= 365) { // Límite de seguridad
                                          let rangeDates = getDatesInRange(currentPeriodStart, clickedDateStr);
                                          ...
                                          if (plannedStart && plannedEnd) {
                                            ...
                                            validDates = combinedDates.filter(dateStr => {
                                              const date = new Date(dateStr);
                                              return date >= startDate && date <= endDate;
                                            });
                                          }
                                  ...
                                  const currentFilter = recurrent ? dateFilter : 'day';
                                  
                                  if (currentFilter === 'day') {
                                    // Modo día: selección normal
                                    if (schedule.includes(clickedDateStr)) {
                                      // Si ya está seleccionada, la removemos
                                      newDates = schedule.filter(d => d !== clickedDateStr);
                                    } else {
                                    ...
                                    const weekdayDates = getSpecificWeekdayInRange(clickedDate);
                                    const allWeekdaySelected = weekdayDates.every((date: string) => schedule.includes(date));
                                    
                                    if (allWeekdaySelected) {
                                      // Si todos los días de esa semana están seleccionados, los removemos
                                      newDates = schedule.filter(d => !weekdayDates.includes(d));
                                    } else {
                                      // Agregar todos los días de esa semana
                                      const uniqueDates = new Set([...schedule, ...weekdayDates]);
                                      newDates = Array.from(uniqueDates);
                                    }
                                  } else if (currentFilter === 'month') {
                                    // Modo mes: seleccionar ese día del mes en todos los meses
                                    const monthdayDates = getSpecificMonthdayInRange(clickedDate);
                                    const allMonthdaySelected = monthdayDates.every((date: string) => schedule.includes(date));
                                    
                                    if (allMonthdaySelected) {
                                      // Si todos los días del mes están seleccionados, los removemos
                                      newDates = schedule.filter(d => !monthdayDates.includes(d));
                                    } else {
                                      // Agregar todos los días del mes
                                      const uniqueDates = new Set([...schedule, ...monthdayDates]);
                                      newDates = Array.from(uniqueDates);
                                    }
                                  }
                              ...
                              modifiers={{
                                ...
                                periodStart: currentPeriodStart ? convertStringArrayToDates([currentPeriodStart]) : [],
                              }}
                              modifiersStyles={{
                                selected: {
                            ...
                            {recurrent && (
                              <div className="px-2">
                                <Select value={dateFilter} onValueChange={(value: 'day' | 'week' | 'month') => setDateFilter(value)}>
                                ...
                                <table className="w-full text-xs">
                                  <thead className="bg-muted/50 sticky top-0 z-10">
                                    <tr>
                                      ...
                                      <th className="text-left p-2 font-medium">Hora fin</th>
                                ...
                                <div className="flex-1 overflow-y-auto">
                                  <table className="w-full text-xs">
                                    <tbody>
                                      ...
                                      const dateOutOfRange = isDateOutOfRange(d);
                                      const dateTimes = getDateTimes(d);
                                      
                                      return (
                                        <tr key={d} className={`hover:bg-accent/30 ${dateOutOfRange ? 'bg-red-50 border-red-200' : ''}`}>
                                          <td className={`p-2 ${dateOutOfRange ? 'text-red-600' : ''}`}>
                                            <div>{localDate.toLocaleDateString()}</div>
                                            {dateOutOfRange && (
                                              <div className="text-xs text-red-500">Fuera del rango</div>
                                            )}
                                          </td>
                                          <td className="p-2">
                              ...
                              <div className="flex items-end gap-2">
                                <div className="w-32">
                                  ...
                                  <Input 
                                    ...
                                    onChange={(e) => setCalendarStartTime(e.currentTarget.value)}
                                ...
                                <div className="w-32">
                                  ...
                                  <Input 
                                    ...
                                    onChange={(e) => setCalendarEndTime(e.currentTarget.value)}
                                    className="h-7 text-xs"
                                  />
                                </div>
                                <Button
